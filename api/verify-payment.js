...
// Called by frontend when user returns from MercadoPago with ?payment=success
// Acts as fallback when webhook doesn't fire (common in sandbox)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (existing.exists) {
      // Already processed — just return success so frontend knows
    ...
    // Security: verify uid matches the payment's external_reference
    if (refUid !== uid) {
