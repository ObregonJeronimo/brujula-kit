    ...
    <div style={{animation:"fi .3s ease",maxWidth:900,margin:"0 auto"}}>
        ...
        {patient && <div style={{background:"#fff",borderRadius:12,border:"1px solid "+K.bd,padding:24,marginBottom:16}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <div>
              ...
              <input type="date" value={evalDate} onChange={function(e){setEvalDate(e.target.value)}} style={{width:"100%",padding:"10px 12px",border:"1px solid "+K.bd,borderRadius:8,fontSize:14}} />
            </div>
            <div>
              <label style={{fontSize:12,fontWeight:600,color:K.mt,display:"block",marginBottom:4}}>{"Derivado por"}</label>
              <input value={derivado} onChange={function(e){setDerivado(e.target.value)}} placeholder="Nombre del derivador" style={{width:"100%",padding:"10px 12px",border:"1px solid "+K.bd,borderRadius:8,fontSize:14}} />
            </div>
          </div>
      ...
      {step===1 && <div>
        <div style={{background:"#f3e8ff",border:"1px solid #d8b4fe",borderRadius:8,padding:"12px 16px",marginBottom:20,fontSize:12,color:"#6b21a8",lineHeight:1.6}}>
          <strong>{"\u2139\ufe0f Instrucciones:"}</strong>{" Para cada grupo, presente los est\u00edmulos. Marque "}<strong>{"S\u00ed"}</strong>{" si reconoce la diferencia o "}<strong>{"No"}</strong>{" si no la reconoce."}
        </div>
          ...
          return <div key={group.id} style={{background:"#fff",borderRadius:12,border:"1px solid "+K.bd,marginBottom:16,overflow:"hidden"}}>
            ...
            <div style={{padding:0}}>
              <div style={{display:"grid",gridTemplateColumns:"50px 1fr 80px 80px 1fr",gap:0,background:"#f8fafc",padding:"8px 16px",fontSize:11,fontWeight:700,color:K.mt,borderBottom:"1px solid #f1f5f9"}}>
                <span>{"L\u00e1m."}</span><span>{"Est\u00edmulos"}</span><span style={{textAlign:"center"}}>{"Reconoce"}</span><span></span><span>{"Obs."}</span>
              </div>
              {group.items.map(function(item){
                ...
                return <div key={item.lam} style={{borderBottom:"1px solid #f1f5f9"}}>
                  <div style={{display:"grid",gridTemplateColumns:"50px 1fr 80px 80px 1fr",gap:0,padding:"10px 16px",background:bgRow,alignItems:"center"}}>
                    ...
                    <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{item.est.map(function(word, idx){ return <span key={idx} style={{padding:"2px 7px",borderRadius:4,fontSize:11,background:"#f1f5f9",color:"#334155"}}>{word}</span>; })}</div>
        ...
        <div style={{marginTop:16}}>
          ...
          <textarea value={obs} onChange={function(e){setObs(e.target.value)}} rows={3} placeholder="Notas adicionales..." style={{width:"100%",padding:"10px 12px",border:"1px solid "+K.bd,borderRadius:8,fontSize:13,marginTop:4,resize:"vertical",fontFamily:"inherit"}} />
        </div>
      ...
      {step===2 && <div>
        ...
        {report && <div style={{marginBottom:20}}>
          ...
          <div ref={reportRef} style={{background:"#fff",borderRadius:12,border:"1px solid "+K.bd,padding:24}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,paddingBottom:12,borderBottom:"2px solid "+K.bd}}>
              <div>
                <div style={{fontSize:10,color:K.mt,fontWeight:600,textTransform:"uppercase",letterSpacing:1}}>{"Informe Fonoaudiol\u00f3gico \u2014 Reco. Fonol\u00f3gico (PEFF-R 3.5)"}</div>
        ...
        {showTech && results && <div>
          ...
          <div style={{background:"#fff",borderRadius:12,border:"1px solid "+K.bd,padding:20,marginBottom:20}}>
            <h3 style={{fontSize:15,fontWeight:700,color:K.ac,marginBottom:12}}>{"Resultados por grupo"}</h3>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {results.groupResults.map(function(g){
                ...
                return <div key={g.id} style={{padding:"10px 14px",borderRadius:8,border:"1px solid "+(isOk?"#bbf7d0":"#fecaca"),background:isOk?"#f0fdf4":"#fef2f2",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div><span style={{fontWeight:800,color:isOk?"#059669":"#dc2626",marginRight:6}}>{g.id}</span><span style={{fontSize:12}}>{g.label}</span></div>
          ...
          {results.errorGroups.length > 0 && <div style={{background:"#fff",borderRadius:12,border:"1px solid "+K.bd,padding:20,marginBottom:20}}>
            ...
            {results.errorGroups.map(function(g){
              ...
              return <div key={g.id} style={{padding:"12px 14px",background:"#fef2f2",borderRadius:8,marginBottom:8}}>
                ...
                <div style={{fontSize:11,color:"#7f1d1d",marginTop:2}}>{"Items no reconocidos: "+failed.map(function(it){return "L\u00e1m. "+it.lam}).join(", ")}</div>
              </div>;
            })}
          </div>}
          {results.errorGroups.length === 0 && <div style={{background:"#dcfce7",borderRadius:12,padding:24,textAlign:"center",marginBottom:20}}><span style={{fontSize:28}}>{"\u2705"}</span><p style={{fontSize:14,fontWeight:600,color:"#059669",marginTop:8}}>{"Reconocimiento fonol\u00f3gico adecuado."}</p></div>}
        </div>}
