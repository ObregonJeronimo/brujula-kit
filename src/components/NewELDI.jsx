...
export default function NewELDI({onS,nfy,userId}){
  ...
  const[pd,sPd]=useState({pN:"",birth:"",eD:new Date().toISOString().split("T")[0],sch:"",ref:"",obs:"",dni:""});
  ...
  // When patient is selected from lookup, fill in patient data
  var handlePatientSelect = function(pac){
    setSelectedPatient(pac);
    if(pac){
      sPd(function(p){ return Object.assign({},p,{
        pN: pac.nombre || "",
        birth: pac.fechaNac || "",
        sch: pac.colegio || "",
        dni: pac.dni || ""
        ...
        return<div key={range} style={{marginBottom:18}}>
          <div style={{background:"#ccfbf1",padding:"6px 12px",borderRadius:6,fontSize:12,fontWeight:600,color:"#0d9488",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:6}}>
            ...
            <div style={{display:"flex",gap:4}}>
              ...
              <button onClick={()=>markSection(gi,"clear")} title="Limpiar seccion"
  ...
  const NOTA_TEXT = "Nota: La clasificacion se basa en un analisis criterial (comparacion con hitos esperados por edad), no en baremos normativos. Cortes: >=90% = Normal, 75-89% = En Riesgo, 50-74% = Retraso Moderado, menos de 50% = Retraso Significativo.";
    ...
    <div style={{background:"#fff",borderRadius:12,padding:28,border:"1px solid #e2e8f0"}}>
      ...
      {content==="patient"&&<div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          ...
          <div style={{gridColumn:"1/-1"}}><label style={{fontSize:12,fontWeight:600,color:K.mt,display:"block",marginBottom:4}}>Derivado por</label><input value={pd.ref} onChange={e=>sPd(p=>({...p,ref:e.target.value}))} style={I} placeholder="Profesional"/></div>
        </div>
        ...
        <div style={{marginTop:20,padding:16,background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:10}}>
          ...
          <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
            <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:14}}><input type="checkbox" checked={evalRec} onChange={e=>setEvalRec(e.target.checked)} style={{width:18,height:18,accentColor:"#0d9488"}}/>{"\ud83d\udd0a Comprension Auditiva (Receptivo)"}</label>
            <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:14}}><input type="checkbox" checked={evalExp} onChange={e=>setEvalExp(e.target.checked)} style={{width:18,height:18,accentColor:"#0d9488"}}/>{"\ud83d\udde3\ufe0f Comunicacion Expresiva"}</label>
          </div>
          {!evalRec&&!evalExp&&<div style={{marginTop:8,color:"#dc2626",fontSize:12,fontWeight:600}}>{"Debe seleccionar al menos un area"}</div>}
        </div>
        <div style={{display:"flex",justifyContent:"flex-end",marginTop:20}}><Bt pr onClick={()=>{if(!selectedPatient){nfy("Busque y seleccione un paciente por DNI","er");return}if(!evalRec&&!evalExp){nfy("Seleccione al menos un area","er");return}setDirty(true);sS(2)}}>{"Siguiente \u2192"}</Bt></div>
      </div>}

      {content==="rec"&&<div>{RI(REC,"AC")}<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:14,paddingTop:14,borderTop:"1px solid #e2e8f0"}}><span style={{fontSize:13,color:K.mt}}>Logrados: <b style={{color:"#0d9488"}}>{rR.logrado}</b>/55 {"\u00b7"} Sin evaluar: <b style={{color:"#f59e0b"}}>{rR.noEvaluado.length}</b></span><div style={{display:"flex",gap:8}}><Bt onClick={()=>sS(step-1)}>{"\u2190 Atras"}</Bt><Bt pr onClick={()=>sS(step+1)}>{"Siguiente \u2192"}</Bt></div></div></div>}

      {content==="exp"&&<div>{RI(EXP,"EC")}<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:14,paddingTop:14,borderTop:"1px solid #e2e8f0"}}><span style={{fontSize:13,color:K.mt}}>Logrados: <b style={{color:"#0d9488"}}>{rE.logrado}</b>/55 {"\u00b7"} Sin evaluar: <b style={{color:"#f59e0b"}}>{rE.noEvaluado.length}</b></span><div style={{display:"flex",gap:8}}><Bt onClick={()=>sS(step-1)}>{"\u2190 Atras"}</Bt><Bt pr onClick={()=>sS(step+1)}>{"Resultados \u2192"}</Bt></div></div></div>}

      {content==="result"&&(()=>{
        const recRes={label:"Comprension Auditiva",evaluated:evalRec,...rR};
        const expRes={label:"Comunicacion Expresiva",evaluated:evalExp,...rE};
