  ...
  return (
    <div style={{ animation: "fi .3s ease", width: "100%", maxWidth: 900 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        <div style={{ textAlign: "center" }}><div style={{ fontSize: 20, fontWeight: 700, color: K.sd }}>{MONTHS[month]} {year}</div><button onClick={goToday} style={{ background: "none", border: "none", color: K.ac, fontSize: 12, cursor: "pointer", fontWeight: 600, marginTop: 2 }}>Ir a hoy</button></div>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>{COLORS.map(c => (<div key={c.id} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: K.mt }}><div style={{ width: 12, height: 12, borderRadius: 3, background: c.hex }} />{c.label}</div>))}</div>

      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", overflow: "hidden", marginBottom: 24 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", background: K.sd }}>{DAYS.map(d => (<div key={d} style={{ padding: "10px 0", textAlign: "center", color: "#5eead4", fontSize: 12, fontWeight: 600 }}>{d}</div>))}</div>
      ...
      {selDay && !showForm && (<div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", padding: 24, marginBottom: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}><button onClick={() => openNewForm(selDay)} style={{ background: K.ac, color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>+ Nueva cita</button><button onClick={() => setSelDay(null)} style={{ background: "#f1f5f9", border: "none", padding: "8px 14px", borderRadius: 8, fontSize: 13, cursor: "pointer", color: K.mt }}>×</button></div>
        </div>
        {dayCitas.length === 0 ? (<p style={{ color: K.mt, fontSize: 13, fontStyle: "italic" }}>{"No hay citas para este día"}</p>) :
          dayCitas.sort((a, b) => (a.hora || "").localeCompare(b.hora || "")).map(c => (<div key={c._fbId} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", background: "#f8faf9", borderRadius: 10, border: "1px solid #e2e8f0", marginBottom: 8, borderLeft: `4px solid ${getColorHex(c.color)}` }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}><span style={{ fontWeight: 700, fontSize: 14 }}>{c.paciente}</span><span style={{ background: getColorHex(c.color) + "22", color: getColorHex(c.color), padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 700 }}>{c.tipo}</span><span style={{ background: c.estado === "realizada" ? "#dcfce7" : c.estado === "cancelada" ? "#fef2f2" : "#fef9c3", color: c.estado === "realizada" ? "#059669" : c.estado === "cancelada" ? "#dc2626" : "#d97706", padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 600 }}>{c.estado}</span></div>
            ...
            <div style={{ display: "flex", gap: 6, flexShrink: 0 }}><button onClick={(e) => { e.stopPropagation(); startEdit(c); }} style={{ background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: 6, padding: "6px 12px", fontSize: 11, fontWeight: 600, cursor: "pointer", color: "#0369a1" }}>Editar</button><button onClick={(e) => { e.stopPropagation(); deleteCita(c._fbId); }} style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 6, padding: "6px 12px", fontSize: 11, fontWeight: 600, cursor: "pointer", color: "#dc2626" }}>Eliminar</button></div>
          </div>))}
      </div>)}
      ...
      {showForm && selDay && (<div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div>
            ...
            <select value={form.tipo} onChange={e => setForm(p => ({ ...p, tipo: e.target.value }))} style={{ ...I, cursor: "pointer" }}>{EVAL_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select>
          </div>
          <div><label style={{ fontSize: 12, fontWeight: 600, color: K.mt, display: "block", marginBottom: 4 }}>Hora</label><input type="time" value={form.hora} onChange={e => setForm(p => ({ ...p, hora: e.target.value }))} style={I} /></div>
          <div><label style={{ fontSize: 12, fontWeight: 600, color: K.mt, display: "block", marginBottom: 4 }}>Estado</label><select value={form.estado} onChange={e => setForm(p => ({ ...p, estado: e.target.value }))} style={{ ...I, cursor: "pointer" }}>{STATUS_LIST.map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>)}</select></div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: K.mt, display: "block", marginBottom: 6 }}>{"Color / Categoría"}</label>
            <div style={{ display: "flex", gap: 8 }}>{COLORS.map(c => (<button key={c.id} onClick={() => setForm(p => ({ ...p, color: c.id }))} title={c.label} style={{ width: 36, height: 36, borderRadius: 8, background: c.hex, border: form.color === c.id ? "3px solid #1e293b" : "2px solid #e2e8f0", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 14, fontWeight: 700, transition: "all .15s" }}>{form.color === c.id ? "✓" : ""}</button>))}</div>
            <div style={{ fontSize: 10, color: K.mt, marginTop: 4 }}>{COLORS.find(c => c.id === form.color)?.label || ""}</div>
          </div>
          <div style={{ gridColumn: "1/-1" }}><label style={{ fontSize: 12, fontWeight: 600, color: K.mt, display: "block", marginBottom: 4 }}>{"Notas clínicas"}</label><textarea value={form.notas} onChange={e => setForm(p => ({ ...p, notas: e.target.value }))} rows={3} style={{ ...I, resize: "vertical" }} placeholder={"Observaciones, objetivos de la sesión..."} /></div>
        </div>
