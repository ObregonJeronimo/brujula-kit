...
export default function AuthScreen({ onDone }) {
  ...
  var handleGoogleSignIn = async function() {
    ...
    try {
      ...
      if (e.code === "auth/account-exists-with-different-credential") {
        setErr("Ya existe una cuenta con este email creada con email/contraseña. Iniciá sesión con tu contraseña.");
      } else {
        setErr("Error con Google: " + e.message);
      }
      setLd(false);
    }
  };
  ...
  var handleResetPassword = async function(ev) {
    ...
    try {
      ...
      setInfo("Se envió un enlace de recuperación a " + resetEmail.trim() + ". Revisá tu bandeja (y spam).");
      setShowReset(false);
    } catch (e) {
      if (e.code === "auth/user-not-found") setErr("No existe una cuenta con ese email.");
      else setErr("Error: " + e.message);
    }
    setLd(false);
  };
    ...
    <div style={{width:"100vw",height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(145deg,#0a3d2f,#0d7363)",fontFamily:"'DM Sans',system-ui,sans-serif"}}>
      <div style={{background:"rgba(255,255,255,.97)",borderRadius:16,padding:"40px 36px",width:420,maxWidth:"92vw",boxShadow:"0 20px 50px rgba(0,0,0,.3)"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:9,marginBottom:6}}><img src="/img/logo_sin_fondo.png" style={{width:36,height:36}} alt="logo" /><span style={{fontSize:26,fontWeight:700,color:"#0a3d2f"}}>{"Brújula KIT"}</span></div>
        ...
        <p style={{textAlign:"center",marginTop:12,fontSize:10,color:"#94a3b8"}}>{"Brújula KIT v6.0"}</p>
      </div>
    </div>
  );
}
