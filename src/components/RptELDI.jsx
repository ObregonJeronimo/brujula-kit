...
export default function RptELDI({ev,isA,onD}){
  ...
  const pdf=()=>{const w=window.open("","_blank");if(!w)return;
    const areaHTML=(area,icon)=>{
      if(!area||!area.evaluated)return`<div style="background:#f1f5f9;padding:16px;border-radius:8px;margin:8px 0"><strong>${icon} ${area?.label||"\u2014"}</strong>: No evaluado en esta sesión</div>`;
      return`<div style="background:#f8faf9;padding:16px;border-radius:8px;border:1px solid #e2e8f0;margin:8px 0">
        ...
        <table style="width:100%"><tr><th>Ítems logrados</th><th>Ítems no logrados</th><th>Sin evaluar</th><th>% Logro (sobre evaluados)</th></tr>
        ...
        ${area.noEvaluado.length>0?`<div style="margin-top:8px;padding:8px;background:#fffbeb;border:1px solid #fde68a;border-radius:6px;font-size:11px"><strong style="color:#92400e">Ítems no evaluados:</strong> ${area.noEvaluado.join(", ")}</div>`:""}
      </div>`;
    };
    ...
    w.document.write(`<h2>2. Resultados \u2014 Puntajes Brutos</h2><div class="warn"><strong>\u26a0 Nota:</strong> Este instrumento no cuenta con baremo normativo publicado oficialmente. Los puntajes son conteos directos de ítems logrados (puntajes brutos). No se calculan puntajes estándar, percentiles ni clasificaciones clínicas derivadas. La interpretación clínica es responsabilidad del profesional evaluador.</div>`);
    w.document.write(areaHTML(recRes,"\ud83d\udd0a"));
    w.document.write(areaHTML(expRes,"\ud83d\udde3\ufe0f"));
    if(allNoEval.length>0)w.document.write(`<div class="warn">\u26a0 Total de ítems sin evaluar: ${allNoEval.length}. Los resultados son parciales.</div>`);
    w.document.write(`<h2>3. Observaciones Clínicas</h2><div class="ob">${ev.observaciones||"Sin observaciones."}</div>`);
    w.document.write(`<div class="ft"><div class="sg"><div class="ln">Firma Profesional</div><p style="font-size:11px;color:#64748b">Fonoaudiólogo/a</p></div><div class="sg"><div class="ln">Fecha</div><p style="font-size:11px;color:#64748b">${new Date(ev.fechaGuardado).toLocaleDateString("es-CL")}</p></div></div></body></html>`);
  ...
  return(<div style={{width:"100%",maxWidth:900,animation:"fi .3s ease"}}>
    ...
    <div style={{background:"#fff",borderRadius:14,padding:32,border:"1px solid #e2e8f0"}}>
      ...
      <div style={{background:"#fef3c7",border:"1px solid #fde68a",borderRadius:10,padding:16,marginBottom:20}}>
        ...
        <div style={{fontSize:12,color:"#78350f",lineHeight:1.6}}>Este instrumento no cuenta con baremo normativo publicado. Los puntajes mostrados son puntajes brutos (conteo directo). No se calculan puntajes estándar, percentiles ni clasificaciones derivadas. La interpretación es responsabilidad del profesional.</div>
      </div>
      ...
      <h3 style={{fontSize:16,fontWeight:700,color:"#0a3d2f",marginBottom:10,paddingBottom:8,borderBottom:"2px solid #ccfbf1"}}>Observaciones Clínicas</h3>
