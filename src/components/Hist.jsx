  ...
  return (
    <div style={{ width: "100%", animation: "fi .3s ease" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        {[["all", "Todas"], ["eldi", "ELDI"], ["peff", "PEFF"]].map(([id, lb]) =>
          <button key={id} onClick={() => sTab(id)} style={{ padding: "6px 14px", borderRadius: 6, border: tab === id ? "2px solid #0d9488" : "1px solid #e2e8f0", background: tab === id ? "#ccfbf1" : "#fff", color: tab === id ? "#0d9488" : "#64748b", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>{lb}</button>)}
      </div>
      ...
      {f.length === 0 ? <div style={{ background: "#fff", borderRadius: 12, padding: 40, textAlign: "center", border: "1px solid #e2e8f0", color: K.mt }}>Sin resultados.</div> :
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {f.map(ev => {
            const isP = ev._t === "peff";
            const bg = isP ? { b: "#ede9fe", c: "#7c3aed", l: "PEFF" } : { b: "#ccfbf1", c: "#0d9488", l: "ELDI" };
            return (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  {isA && (cf === (ev._fbId || ev.id) ?
                    <div style={{ display: "flex", gap: 4 }}>
                      <button onClick={() => { onD(ev._fbId, isP ? "peff_evaluaciones" : "evaluaciones"); sC(null); }} style={{ background: "#dc2626", color: "#fff", border: "none", padding: "5px 10px", borderRadius: 5, fontSize: 11, cursor: "pointer", fontWeight: 600 }}>Sí</button>
                      <button onClick={() => sC(null)} style={{ background: "#f1f5f9", border: "none", padding: "5px 10px", borderRadius: 5, fontSize: 11, cursor: "pointer" }}>No</button>
                    </div> :
                    <button onClick={() => sC(ev._fbId || ev.id)} style={{ background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca", padding: "5px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11 }}>🗑</button>)}
