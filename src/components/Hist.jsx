...
export default function Hist({ es, pe, re, de, onV, onVP, onVR, onVD, isA, onD }) {
  ...
  return (
    <div style={{width:"100%",animation:"fi .3s ease"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
        {[["all","Todas"],["eldi","ELDI"],["peff","PEFF"],["rep","Rep.Palabras"],["disc","Disc.Fonol."]].map(function(x){
          var id=x[0], lb=x[1];
      ...
      {f.length === 0 ? <div style={{background:"#fff",borderRadius:12,padding:40,textAlign:"center",border:"1px solid #e2e8f0",color:K.mt}}>Sin resultados.</div> :
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {f.map(function(ev){
            ...
            var isR = ev._t === "rep";
            var isD = ev._t === "disc";
            var bg = isD ? { b:"#fef3c7", c:"#d97706", l:"DISC" } : isR ? { b:"#dbeafe", c:"#2563eb", l:"REP" } : isP ? { b:"#ede9fe", c:"#7c3aed", l:"PEFF" } : { b:"#ccfbf1", c:"#0d9488", l:"ELDI" };
            var colName = isD ? "disc_evaluaciones" : isR ? "rep_evaluaciones" : isP ? "peff_evaluaciones" : "evaluaciones";
            var viewFn = isD ? onVD : isR ? onVR : isP ? onVP : onV;
            return (
              <div key={ev._fbId||ev.id} style={{background:"#fff",borderRadius:10,padding:"14px 20px",border:"1px solid #e2e8f0",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  {(isP||isR||isD) && ev.resultados && <span style={{padding:"3px 10px",borderRadius:14,background:bg.b,color:bg.c,fontSize:12,fontWeight:600}}>{ev.resultados.severity||(ev.resultados.pct+"%")}</span>}
