  ...
  var renderWordCell = function(phonId, posId, word){
    ...
    return <div style={{marginBottom:6}}>
      ...
      <div style={{display:"flex",gap:4,alignItems:"center"}}>
        <button onClick={function(){ toggleOk(key); }} style={{width:30,height:30,borderRadius:6,flexShrink:0,border:isOk?"2px solid #059669":"1px solid "+K.bd,background:isOk?"#dcfce7":"#fff",color:isOk?"#059669":K.mt,fontSize:14,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{isOk?"\u2713":""}</button>
        <input value={hasError?val:""} placeholder="error..." onChange={function(e){ var v=e.target.value.trim(); if(v==="") setResponse(key,""); else setResponse(key,v); }} onFocus={function(){ if(isOk) setResponse(key,""); }} style={{flex:1,padding:"5px 8px",border:"1px solid "+(hasError?"#fca5a5":K.bd),borderRadius:6,fontSize:12,background:hasError?"#fef2f2":"#fff",color:hasError?"#dc2626":"#1e293b",minWidth:0}} />
      </div>
    </div>;
  };

  return (
    <div ref={mainRef} style={{animation:"fi .3s ease",maxWidth:1200,margin:"0 auto"}}>
      ...
      {step===1 && <div>
        <div style={{display:"flex",gap:4,marginBottom:16,overflowX:"auto",paddingBottom:4}}>{catGroups.map(function(cg,i){ var prog=catProgress(cg.id,responses),active=catIdx===i,complete=prog.answered===prog.total&&prog.total>0; return <button key={cg.id} onClick={function(){setCatIdx(i);scrollTop();}} style={{padding:"8px 12px",borderRadius:8,border:active?"2px solid "+K.ac:"1px solid "+K.bd,background:active?K.al:complete?"#dcfce7":"#fff",color:active?K.sd:complete?"#059669":K.mt,fontSize:11,fontWeight:active?700:500,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>{cg.title}{prog.total>0&&<span style={{display:"block",fontSize:9,marginTop:2}}>{prog.answered+"/"+prog.total}</span>}</button>; })}</div>
        {(function(){ var cg=catGroups[catIdx]; if(!cg) return null; return <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h2 style={{fontSize:17,fontWeight:700,margin:0}}>{cg.title}</h2><button onClick={function(){markAllCatOk(cg.id)}} style={{padding:"6px 14px",borderRadius:6,border:"1px solid #059669",background:"#dcfce7",color:"#059669",fontSize:11,fontWeight:600,cursor:"pointer"}}>{"\u2713 Todo correcto"}</button></div>
          <div style={{background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:12,color:"#0369a1",lineHeight:1.6}}><strong>{"\u2139\ufe0f"}</strong>{" Marque \u2713 si repiti\u00f3 correctamente. Si hay error, escriba la producci\u00f3n."}</div>
          {cg.phonRows.map(function(pr){ var isExp=(patientAge/12)>=pr.age; return <div key={pr.phonId} style={{background:"#fff",borderRadius:12,border:"1px solid "+K.bd,marginBottom:14,overflow:"hidden"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",background:isExp?"#f0fdf4":"#fffbeb",borderBottom:"1px solid "+K.bd}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:18,fontWeight:700,color:K.sd}}>{pr.phoneme}</span></div>
              <button onClick={function(){markPhonOk(pr.phonId,pr.posWords)}} style={{padding:"4px 10px",borderRadius:6,border:"1px solid #bbf7d0",background:"#dcfce7",color:"#059669",fontSize:10,fontWeight:600,cursor:"pointer"}}>{"\u2713 Todo"}</button>
            </div>
            <div style={{padding:"12px 16px"}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:12}}>{POSITIONS.map(function(pos){ var ws=pr.posWords[pos.id]||[],has=ws.length>0; return <div key={pos.id}><div title={pos.full} style={{fontSize:11,fontWeight:700,color:has?K.ac:"#cbd5e1",textAlign:"center",padding:"4px 0",borderBottom:"2px solid "+(has?K.ac:"#f1f5f9"),marginBottom:8,cursor:"help"}}>{pos.label+" \u24d8"}</div>{has?ws.map(function(w){return <div key={w}>{renderWordCell(pr.phonId,pos.id,w)}</div>}):<div style={{textAlign:"center",color:"#cbd5e1",fontSize:11,padding:8}}>{"\u2014"}</div>}</div>; })}</div></div>
          </div>; })}
