...
export default function App() {
  ...
  return (
    <div className="ek-layout">
      ...
      {forceConfig && !mobile && <div className="ek-force-backdrop">
      ...
      <aside className={"ek-sidebar"+(mobile?" is-mobile":"")}>
        ...
        <div className="ek-sidebar-footer">{!mobile&&<div className="ek-sidebar-session">{"Sesión: "}<b className="ek-sidebar-session-name">{profile.username}</b>{isAdmin&&<span className="ek-sidebar-admin-badge">ADMIN</span>}</div>}{!mobile&&<div className="ek-sidebar-credits">{"Créditos: "}<b className={"ek-sidebar-credits-value"+(profile.creditos>0?"":" is-empty")}>{isAdmin?"\u221e":(profile.creditos||0)}</b></div>}<button onClick={handleLogout} className={"ek-sidebar-logout"+(mobile?" is-mobile":"")}><span className="ek-sidebar-logout-ico">{icons.logout}</span>{!mobile&&<span>{"Cerrar sesión"}</span>}</button></div>
      </aside>
      <main id="main-scroll" className={"ek-main"+(mobile?" is-mobile":"")} style={{"--ek-main-bg":"url(\""+DINO_BG+"\")"}}>
        ...
        {view==="dash"&&<Dashboard TC={TC} allEvals={allEvals} onT={function(){navTo("tools")}} onView={viewReport} ld={loading} profile={profile} isAdmin={isAdmin} userId={authUser?.uid} nfy={nfy} onCalendar={function(){navTo("calendario")}} onStartEval={startEval} onBuyCredits={goToPremium} userSettings={userSettings} />}
        {view==="tools"&&<Tools TC={TC} onSel={startEval} credits={isAdmin?999:(profile.creditos||0)} onBuy={goToPremium} enabledTools={enabledTools} toolsConfig={toolsConfig} userId={authUser?.uid} onResumeDraft={resumeDraft} allEvals={allEvals} nfy={nfy} therapistInfo={profile?.reportHeader} deductCredit={deductCredit} isAdmin={isAdmin} onReload={loadEvals} />}
        <Suspense fallback={LazyFallback}>
          ...
          {view==="premium"&&<PremiumPage TC={TC} profile={profile} authUser={authUser} nfy={nfy} onBack={function(){sV("dash")}} />}
