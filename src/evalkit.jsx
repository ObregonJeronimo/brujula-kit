...
function withTimeout(promise, ms, label) { return Promise.race([promise, new Promise((_, reject) => setTimeout(() => reject(new Error(`TIMEOUT después de ${ms}ms en: ${label}`)), ms))]); }
...
export default function App(){
  ...
  const navTo=(v)=>{
    if((view==="newELDI"||view==="newPEFF")&&v!==view){
      if(!window.confirm("¿Está seguro que desea salir? Los cambios NO se guardarán."))return;
    }
    sV(v);sS(null);
  };
  ...
  const go=async ev=>{ev.preventDefault();sl(true);se("");setInfo("");
    try{
      ...
      if(users.length===0){
        ...
        try{await withTimeout(addDoc(collection(db,"usuarios"),{usuario:"CalaAdmin976",contrasena:"BrujulaKit2025!"}),10000,"addDoc")}catch(addErr){se("Error creando usuario: "+addErr.message);sl(false);return}
